...
移动端优先的 Pokémon Champions Regulation Set M-A 对战助手 PWA。当前分支正在推进“环境优先”重构：默认入口提供环境榜单与开发样例队伍，队伍、伤害计算、速度线、规则图鉴和本地数据管理继续保留为完整工具闭环。
...
- 环境：单打 / 双打切换、宝可梦榜、完整榜单、宝可梦环境详情、常用招式、携带道具、常见队友、相关样例队伍
- 环境样例队伍：当前为开发 seed 占位数据，支持导入配置和队报链接；真实高分 / 使用率数据接入前不包装成正式环境统计
- 环境数据地基：环境榜单和样例队伍已收束为 `EnvironmentDataset` 数据包，并在进入 UI 前经过引用与数值审计
...
环境页当前使用开发样例数据：宝可梦榜的占比、队伍数、常用招式、携带道具、常见队友和样例队伍均为本地 seed 占位，不代表真实使用率或真实高分样本。数据会先通过 `src/lib/environmentDataset.ts` 审计，未知 Pokémon / 招式 / 道具引用会被报告并从 UI 数据中剔除。
