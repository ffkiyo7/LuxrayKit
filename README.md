...
[![PWA](https://img.shields.io/badge/PWA-ready-38BDF8)](./public/manifest.webmanifest)
...
Luxray Kit 是一个面向 Pokémon Champions 玩家的非官方对战辅助工具。它以移动端 PWA 形态设计，围绕 Regulation Set M-A 的日常查询与配队流程，提供环境快照、队伍管理、伤害参考计算、速度线对比和规则图鉴。

项目仍在持续开发中。当前版本已经接入 PokeDB Open Data 上位构筑样本作为环境页数据来源；伤害计算仍属于实验性近似结果，不应视为官方结论或赛事依据。

> 计划分发域名：`luxraykit.com`
...
- 伤害计算：攻击方 / 防守方独立配置、队伍成员带入、图鉴搜索、战斗条件和结果卡
...
| 伤害计算 | 实验性近似 | 使用 `@smogon/calc` Gen9 主线公式近似，并代入项目采集的 Champions 招式参数与 SP 能力值 |
...
如有版权或商标方面的问题，请通过 [GitHub Issues](https://github.com/ffkiyo7/Pokemon-champions-Tool/issues) 联系作者。
