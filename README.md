...
- 队伍码写入、导入带入与详情页复制
- **分享链接**：队伍详情一键生成 `#/t/<code>` 链接（有系统分享面板就调分享面板，没有就复制到剪贴板）；对方打开先看预览再决定导不导入，跨规则失效的配置会逐条列出而不是悄悄丢掉
...
- 主题切换
- **站内留言**：一个表单直接写给我们，**不需要 GitHub 账号**；留言不公开，服务端存在 Cloudflare。「关于」显示规则、数据版本与构建标识（留言会自动带上），并保留一个 GitHub issue 出口
- 匿名使用统计开关（默认开启，只记录页面模式 / 是否 PWA / 主题 / 国家，无 cookie 与标识符）
- 本地数据导出 / 导入
- 离线缓存说明
- 清除本地数据
...
| 可深链 / 可返回 | 导航状态写在 URL hash 里，Android 物理返回键在 App 内层层后退而不是直接退出；刷新停在当前页，任何页面都能收藏和分享 |
