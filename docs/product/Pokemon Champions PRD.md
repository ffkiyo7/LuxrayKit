...
- Pokémon、招式、道具、特性入口。
- 当前规则 Pokémon 搜索、最多双属性筛选、Mega-only 筛选与详情页。
- Pokémon 列表使用紧凑卡片，展示头像、图鉴编号、中文名、日文 / 英文名和属性。
- 详情页展示正方形头像、身高 / 体重、特性、单色种族值、属性弱点 / 抵抗 / 免疫关系、learnset 和 Mega 形态。
- 可学会招式支持搜索、单属性筛选、按属性 / 威力排序与升序 / 降序切换。
- 招式 catalog 支持搜索与单属性筛选；道具 / 特性 catalog 使用全宽搜索栏。
- 道具本地图片快照。
