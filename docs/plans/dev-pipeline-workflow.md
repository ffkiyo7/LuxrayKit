...
| 强模型 UI | 新建一个 codex-bot。owner 发送 `/dispatch task:<任务>`，在频道确认卡中选择“在 Codex 中继续”或“在 Claude 中继续”后得到原生 public Thread。普通 Hermes 聊天不改变。 |
...
1. owner 照常在 luxraykit-dev 与 Hermes 交流；需要升级任务时，使用 `/dispatch task:<任务>` 发确认卡，并在其中选择 Codex 或 Claude 建 Thread。
...
| DPH-04 | Discord interaction/UI | `/dispatch` 确认卡/Modal、Thread、真实置顶状态卡、owner-only 输入和日志脱敏完成。 |
