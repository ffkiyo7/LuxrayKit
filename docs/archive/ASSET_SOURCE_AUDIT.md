> 已归档（2026-09-23）：2026-06-18 的一次性审计快照，高优先级 #1–#4 与花叶蒂修正均已落地（见文末「处理进度」）。第二、四节的可用性实测与 unknown 清单仅代表当时状态；来源与许可风险的现行记录是 `src/data/seed/regMA/metadata.ts` 的 `dataSourceManifest` 与根目录 `THIRD_PARTY_NOTICES.md`。中优先级建议 #5（机器可读资产来源 manifest）、#6（PokéBase 冗余 / HTML 快照）截至归档仍未实施，可作后续参考。
