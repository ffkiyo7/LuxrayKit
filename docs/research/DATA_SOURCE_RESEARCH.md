...
- PokeDB Open Data 不包含训练家名、队报链接、性格、SP、完整招式配置和样本登记时间；招式使用率通过维护脚本额外解析 Pokémon 详情页 `data-move-detail`，当前覆盖单打 / 双打各前 50 Pokémon；队报链接和训练家名来自 PokeDB trainer/list 页面解析，当前保留单打 24 条、双打 19 条完整队伍骨架样本。
...
- Open Data 中不存在的字段不得伪造为统计结果；常用招式必须来自 PokeDB 详情页解析后的 `moveStats`，队报链接必须来自 trainer/list 或人工明确补充。上位构筑导入只能自动填入可确定的 Pokémon / 道具 / Mega 形态 / 单一特性，不能用聚合招式统计、默认性格或默认 SP 冒充原队配置。
...
- 扩展队报样本来源：trainer/list 解析已扩展为单打 24 条、双打 19 条；后续重点转向补充文章标题，或按队报原文进一步解析完整配置。
