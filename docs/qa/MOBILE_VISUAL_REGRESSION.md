...
- 浅色主题没有视觉基线。
- **速度线头部文案不在基线内**：用例 17 截图前 `scrollBy(0, 120)` 把头部滚出视口，这行怎么改都不会被视觉门禁抓到（2026-09-07 `speedTierSeason` 3→5 后重跑 `visual-baseline.yml`，18 张全部报「已一致」）。2026-09-20 该行由「PokeDB M-{season} 静态参照 · 上下滑动看档位」改为「热门速度线参照 · 上下滑动看档位」，不再渲染赛季号，`speedTierSeason` 也不再被 `SpeedPage.tsx` 引用——原先「赛季号回归无人兜底」的隐患随之消失（`SpeedPage.test.tsx` 其实从未断言过这行）。
