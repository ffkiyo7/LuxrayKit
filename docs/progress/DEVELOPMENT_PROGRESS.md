...
- ✅ **Mega 具甲武者特性定论 = Tough Claws**（2026-09-10，PokeDB M-6）：`/pokemon/show/0768-00?season=6` 的形态 payload 直接写 `"0768-01"`（メガグソクムシャ）`ability_key: 181` / `かたいツメ`，与 Bulbapedia 一致；PokémonDB 与 `@smogon/calc` 报的 Emergency Exit 确认是 slot-0 继承假象。落库值不变，依据补进 `mega-catalog-mc.ts` 头注释。其余五个同源核实：292 きれあじ / 26 ふゆう / 317 はどうのぼうご / 184 スカイスキン / 270 ねつこうかん。
- ✅ **6 块 Mega 石日文名已从 PokeDB 读出、`aura-guard` 的 resource key 已补**（2026-09-10）：六个名字与此前推断值**逐字相同**，三块 Z 石一律**全角 Ｚ**（アブソルナイトＺ / ガブリアスナイトＺ / ルカリオナイトＺ，另 ボーマンダナイト / グソクムシャナイト / セグレイブナイト）；半角 Z 行保留为防御性别名。`aura-guard` = **317**（メガルカリオＺ `0448-02` 的 `ability_key`，PokeDB 自有扩展号段，与表内 309–311 同源），不再留空。
- ✅ **Task 9 验收边界「M-C 开赛后跑一次真实 PokeDB 刷新」已满足**（2026-09-10）：`data:pokedb:environment` 对 M-6 实跑（singles 241 榜 / doubles 215 榜，各取 60 张详情页），**零容忍审计两个规则全零**；静态兜底 `battles.*.season` 推进到 **M-6**，`speedTierSeason` → **6**。首跑暴露三处真实缺口，均按可核实来源补齐：
  - **6 个招式键 + 4 个特性键**（756 コートチェンジ / 778 ドラムアタック / 780 かえんボール / 862 きょけんとつげき / 863 さいきのいのり / 892 でんこうそうげき；194 ききかいひ / 227 サイコメイカー / 229 グラスメイカー / 236 リベロ）——都是主线既有资源，PokeDB 用的就是 PokeAPI 编号，逐个用日文名与 PokeAPI 同号条目对上后才写进 `pokedbResourceKeyMap.ts`。
  - **队伍样本会被清零**：`/trainer/list` 现在对 **M-5 与 M-6 都报 0件**（M-1 45 件 / M-3 25 件 / M-4 31 件仍在），而脚本默认取「上一赛季」。`update-pokedb-environment.mjs` 改为默认赛季无队伍时继续往前回溯（显式 `POKEDB_SAMPLE_SEASON` 不受影响），样本从 M-4 取回 24 + 24，与刷新前持平。
  - **两个 M-C 形态掉出速度线**：`ストリンダー (ロー)` / `イエッサン (メス)` 在 catalog 里是与本体同名的独立行，名字索引只留得下第一个，`-01` 切片既匹配不到名字也不能按 dex 回退（爱管侍雌性种族速度 85 ≠ 雄性 95）。`speedTier.ts` 加一张显式 `CATALOG_FORM_ALIASES`（`849-01` / `876-01`），门禁用例 `resolves every pokemon in the speed snapshots` 由红转绿。
- 🎮 **`speedTier.test.ts` 的 Mega Z 占位守卫暂不替换**：M-6 是真正的 M-C 赛季，但 PokeDB 速度线**至今没有任何 Mega Z 切片**（两个规则的快照里 `'151'` 档位为 0），守卫这次是在真 M-C 数据上成立、而非因赛季缺席，故保留并把注释改写成现状。等 PokeDB 开始列 Z Mega 再换成同文件的 resolution 检查。
...
- **生成产物刷新到 M-5**：静态环境快照 → 2026-09-07 07:18；`speedTiers.ts` 从 M-3 → M-5（`speedTierSeason` 3→5，SpeedPage 头部文案随之改变）。视觉基线 **未**重建：`visual-baseline.yml` 实测 18 张全部与现状一致，因为速度线用例截图前 `scrollBy(0, 120)` 已把「PokeDB M-{season} 静态参照」头部滚出视口——这条文案不在视觉门禁覆盖范围内，缺口已记入 `docs/qa/MOBILE_VISUAL_REGRESSION.md`。（2026-09-20 起该行改为「热门速度线参照 · 上下滑动看档位」，不再渲染赛季号，`speedTierSeason` 只剩 `speedTiers.ts` 自身导出。）
