...
当前阶段：**环境优先重构分支已跑通主信息架构：默认进入环境页，队伍页列表优先，伤害计算 / 速度线 / 图鉴收束到工具页，我的页承接偏好、备份与缓存管理。下一步重点是继续补齐环境数据来源、更新 PWA 回归覆盖，并在真实数据接入前保持现有工具能力稳定。**
...
- 环境首页：单打 / 双打切换、宝可梦榜、热门高分队伍样本、队报链接、导入配置。
- 完整宝可梦榜：从环境首页进入，点击 Pokémon 进入环境详情，而不是跳传统图鉴页。
- 宝可梦环境详情：展示常用招式、携带道具、常见队友和相关高分队伍，并支持从相关队伍导入配置。
- 高分队伍导入：生成本地队伍副本，保留来源 metadata，跳转队伍列表并高亮新导入队伍。
...
2. 继续接入和审计真实环境数据来源，优先验证使用率、常用招式、携带道具和高分队伍样本的可获得性。
