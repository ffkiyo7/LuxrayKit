...
function prBody(source) {
  return [
    'Generated by the LuxrayKit VPS PokeDB environment refresher.',
    '',
    source === 'worker'
      ? '- Copied from the healthy Worker snapshot (`/api/environment/latest`); PokeDB was not contacted.'
      : '- Crawled PokeDB through the existing `npm run data:pokedb:environment` parser and audit path.',
    '- Zero unknown audit entries and both battle types present (checked before commit).',
...
async function main() {
  ...
  if (!process.argv.includes('--force')) {
    ...
    if (!health.shouldRefresh) {
      console.log('Worker snapshot is healthy and fresh, and the static fallback is within its lag budget. Nothing to do.');
      return;
    }
    if (canCopyFromWorker(health.reason)) source = 'worker';
  } else {
    console.log('Worker environment gate bypassed by --force: crawling PokeDB directly.');
  }

  try {
    if (source === 'worker') {
      console.log('Worker is healthy; copying its snapshot into the static fallback instead of crawling PokeDB.');
      const text = await fetchWorkerSnapshotText();
      await Promise.all(generatedSnapshotPaths.map((path) => writeFile(resolve(ROOT, path), text, 'utf8')));
    } else {
      await run('npm', ['run', 'data:pokedb:environment']);
      assertPublishableSnapshot(JSON.parse(await readFile(resolve(ROOT, generatedSnapshotPaths[1]), 'utf8')));
    }
  } catch (error) {
    // Leave the maintenance clone clean for the next cron run.
    await run('git', ['restore', '--', ...generatedSnapshotPaths]).catch(() => {});
    throw error;
  }
