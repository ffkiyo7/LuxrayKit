...
describe('CalculatorPage', () => {
  ...
  it('swaps attacker and defender configs without changing battle conditions', async () => {
    ...
    expect(sideCard('attacker').textContent).toContain('SP HP 32');
    ...
    expect(sideCard('defender').textContent).toContain('SP HP 32');
  ...
  it('lists the top environment picks under 环境常用 and leaves the config blank', async () => {
    ...
    await user.click(screen.getByRole('button', { name: '环境常用' }));
    await user.click(screen.getByRole('button', { name: '烈咬陆鲨' }));

    expect(sideCard('attacker').textContent).toContain('烈咬陆鲨');
    expect(sideCard('attacker').textContent).toContain('SP 未分配');
  });
});
