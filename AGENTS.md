...
- **两条独立时间轴**。规则类型（Regulation，如 M-A / M-B）决定可用宝可梦、道具等池子；赛季（Season，如 S-1…S-4）是规则周期内的 PokeDB 排位赛季。截至 2026-07，当前为 **M-B 规则下的 S-4 赛季**；具体以 `src/data/seed/regMA/metadata.ts` 的 `currentRuleSet`、`src/data/schedule.ts` 和快照 `seasonLabel` 为准，不要硬编码。
